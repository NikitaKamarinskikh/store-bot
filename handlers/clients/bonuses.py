...
За приглашение нового пользователя вы получите {referral_program_settings.user_acquisition_reward} монет после его 
первой покупки, а так же, приглашённый, сразу получит {referral_program_settings.referral_reward} монет на первую 
покупку.
Кроме того, со всех покупок приглашенного вы будете получать 
{referral_program_settings.referral_order_reward_in_percentages}% в виде монет на свой бонусный счет.
...
async def invite_user(message: types.Message):
    referral_program_settings = load_referral_program_settings_from_json_file()
    await message.answer(
        f'Просто скопируй или перешли сообщение ниже. В нем твоя ссылка, по которой приглашенный сразу получит '
        f'{referral_program_settings.referral_reward} монет на первый заказ, а ты получишь свой бонус '
        f'{referral_program_settings.user_acquisition_reward} монет, после его первого заказа. Кроме того сразу '
        f'же получишь еще и % от его заказа.'
    )
    bot_name = (await message.bot.get_me()).username
    referral_link = f"https://t.me/{bot_name}?start={message.from_user.id}"
    await message.answer(
        f'Получи {referral_program_settings.referral_reward} монет на первый заказ снаряжения и экипировки в '
        f'мастерской BearGear по этой ссылке: {referral_link}'
    )
